...
This was a random sideproject and I wanted to experiment with gleam, so I put some arbitrary constraints in order to attempt to make the algo very optimized *(as these arbitrary constraints also happen to be useful when you have a slow network and for some reason want to update your blogs in milliseconds idk)*.
...
Markfish uses a streaming approach so that the number of requests can be reduced, and essentially serves a different purpose than rsync.
...
We only need two real operations, acting upon a state datastructure *(which is essentially, `List(Chunks), Chunk -> {index, hash}`)*.
...
So intuitively, we will be representing the current input block with pointer `HeadA` and it will be incremented one-by-one.
...
Now, in case of `a4`, we could have replaced a4 (delete then insert), but in case of `a5`, it appears just next after `a6`, so deletion was not optimal. To mitigate this we somehow need to know if `headB` will ever appear in A or not. We cannot look ahead on A, so the only option we have is to defer the decision for later.
...
## The rubberbanding lookup table
...
Anyways, rubberbanding set sounds a lot cooler, and it's implemented as:
...
Gleam doesn't have pointers *(and doesn't need to have, because you can achieve the same thing by using a dequeue)*, and also the lists in gleam are linked lists, so lookups and getting the last element is really inefficient, so the implementation is very very different from the earlier pseudocode.
